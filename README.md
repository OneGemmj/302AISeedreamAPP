...
每次发布前都要递增 `APP_VERSION_CODE`，并让 `APP_VERSION_NAME` 与 Git 标签一致，例如 `v1.1.0` 对应 `APP_VERSION_NAME=1.1.0`。
...
如果 `v1.1.0` 的 APK 没有出现在 GitHub Release，请检查标签是否已经推送：
...
Before each release, increment `APP_VERSION_CODE` and keep `APP_VERSION_NAME` aligned with the Git tag. For example, tag `v1.1.0` should use `APP_VERSION_NAME=1.1.0`.
