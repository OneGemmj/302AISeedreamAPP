...
每次发布前都要递增 `APP_VERSION_CODE`，并让 `APP_VERSION_NAME` 与 Git 标签一致，例如 `v1.1.4` 对应 `APP_VERSION_NAME=1.1.4`。
...
如果 `v1.1.4` 的 APK 没有出现在 GitHub Release，请检查标签是否已经推送：
...
```powershell
gh release upload v1.1.4 app\build\outputs\apk\debug\app-debug.apk --repo OneGemmj/302AISeedreamAPP --clobber
```
